...
formula_path = File.expand_path("../Formula/nauclio.rb", __dir__)
formula = File.read(formula_path)
formula.sub!(/^  version ".*"$/, "  version \"#{version.delete_prefix("v")}\"")
formula.sub!(%r{^  url ".*"$}, "  url \"https://github.com/dbpprt/homebrew-tap/releases/download/#{version}/nauclio-darwin-arm64.tar.gz\"")
